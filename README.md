...
scenarios, and a dynamic takeaway that rewrites itself from the live measures.
...
```
dashboard/  Nable_GTM_Capacity_Dashboard.pbix    the report — 3 pages, 22 tables, 59 measures
model/      Nable_SaaS_GTM_Capacity_Model_v2.xlsx  the 8-tab source model
            Nable_PowerBI_Input.xlsx               12 flattened sheets the report loads
dax/        measures.dax                          all 59 measures in one DEFINE block
            calculated_tables.dax                  the 11 calculated tables
theme/      Nable_Theme.json                       report theme
images/     page exports (add your own — see images/README.md)
```

### Opening it

Open `dashboard/Nable_GTM_Capacity_Dashboard.pbix` in **Power BI Desktop** (Windows).
No data source refresh is needed — the model is imported and self-contained.

### Rebuilding from scratch

1. `Get Data > Excel workbook` → `model/Nable_PowerBI_Input.xlsx` → load all 12 sheets
2. Rename `Scenario_Table` → `Scenario`, `GTM_Capacity_Monthly` → `Capacity Monthly`,
   `Assumptions_Register` → `Assumptions`
3. Run `dax/calculated_tables.dax` block by block via `Modeling > New table`
4. Paste `dax/measures.dax` into DAX query view → **Update model: add new measures**
5. Delete every auto-detected relationship — the model is intentionally relationship-free
6. Apply `theme/Nable_Theme.json`
